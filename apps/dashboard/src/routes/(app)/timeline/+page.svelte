<script lang="ts">
	...
	import { auditDiffLines, isRewrittenMemory } from '$lib/observatory/timeline-audit';
	import {
	...
	const visibleTimeline = $derived.by(() => {
		if (!rewrittenOnly) return timeline;
		return timeline
			.map((day) => ({
				...day,
				memories: day.memories.filter(isRewrittenMemory),
				count: day.memories.filter(isRewrittenMemory).length
			}))
