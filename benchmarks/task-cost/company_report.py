...
def create(bundle, destination, baseline, candidate):
    ...
    with tempfile.TemporaryDirectory(dir=destination.parent) as temp:
        ...
        for arm in (baseline, candidate):
            ...
            rows.append(f"| {arm.replace('|', '/').replace(chr(10), ' ')} | {report['successes']}/{report['planned_tasks']} | "
        ...
        manifest = {"version": "vestige-company-evaluation/v1", "files": {
            name: ledger.digest(root / name) for name in ("comparison.json", "REPORT.md")}}
...
if __name__ == "__main__":
    ...
    create(args.bundle, args.destination, args.baseline, args.candidate)
