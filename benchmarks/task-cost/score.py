...
def score(bundle, arm, case_id, trial, checkout, *, trusted=False, timeout_seconds=300):
    ...
    ledger.require(type(timeout_seconds) is int and 1 <= timeout_seconds <= 3600,
    ...
    ledger.require(not root.is_relative_to(checkout) and not checkout.is_relative_to(root),
    ...
    ledger.require(type(trial) is int and 0 <= trial < contract["repetitions"], "invalid trial")
    ...
    ledger.require(evaluator.suffix == ".py", "scoring requires a frozen Python evaluator")
    for line in (root / "events.jsonl").read_text().splitlines():
        event = ledger.parse_json(line)
        ledger.require(not (event["kind"] == "outcome" and event["arm"] == arm and
                            event["case"] == case_id and event["trial"] == trial),
    ...
    process = subprocess.Popen([sys.executable, "-I", str(evaluator), str(checkout)], cwd=checkout,
    ...
    ledger.require(after["events_sha256"] == report["events_sha256"], "ledger changed during evaluation")
    receipt = {"version": "vestige-task-evaluation/v1", "status": status,
               "exit_code": process.returncode, "elapsed_ms": elapsed_ms,
               "timeout_seconds": timeout_seconds, "evaluator_sha256": ledger.digest(evaluator),
               "runner_sha256": ledger.digest(__file__), "contract_sha256": report["contract_sha256"],
    ...
    with receipt_path.open("x") as stream:
        ...
        stream.write("\n")
    ledger.record(root, {"id": f"evaluation-{identity}", "kind": "outcome", "arm": arm,
                        "case": case_id, "trial": trial, "status": status,
                        "evidence": {"path": receipt_path.name, "sha256": ledger.digest(receipt_path)}})
    return receipt
...
if __name__ == "__main__":
    ...
    print(json.dumps(score(args.bundle, args.arm, args.case, args.trial, args.checkout,
                           trusted=args.execute_trusted_evaluator, timeout_seconds=args.timeout_seconds), indent=2))
