...
def artifact(root, reference):
    ...
    require(re.fullmatch(r"[0-9a-f]{64}", expected) is not None, "invalid artifact hash")
...
def normalize(format_name, usage):
    ...
    if format_name == "openai_responses":
        ...
        tokens.update(uncached_input=total - cached, cached_input=cached,
                      output=integer(usage.get("output_tokens"), "output_tokens"))
        if "total_tokens" in usage:
            require(integer(usage["total_tokens"], "total_tokens") == total + tokens["output"],
        ...
        tokens.update(uncached_input=integer(usage.get("input_tokens"), "input_tokens"),
                      output=integer(usage.get("output_tokens"), "output_tokens"))
        if "cache_read_input_tokens" not in usage or "cache_creation_input_tokens" not in usage:
            return None
        tokens["cached_input"] = integer(usage["cache_read_input_tokens"], "cache_read_input_tokens")
        created = integer(usage["cache_creation_input_tokens"], "cache_creation_input_tokens")
        creation = usage.get("cache_creation")
        if created or creation is not None:
            if not isinstance(creation, dict):
                return None  # Cannot guess which TTL was billed.
            tokens["cache_write_5m"] = integer(creation.get("ephemeral_5m_input_tokens"), "cache_write_5m")
            tokens["cache_write_1h"] = integer(creation.get("ephemeral_1h_input_tokens"), "cache_write_1h")
            require(tokens["cache_write_5m"] + tokens["cache_write_1h"] == created,
...
def freeze(bundle):
    ...
    lock = {"version": VERSION, "contract_sha256": digest(bundle / "contract.json"),
...
def validate_contract(root, contract):
    ...
    require(contract.get("evidence_kind") in ("synthetic", "provider_exports"), "evidence_kind required")
    require(contract.get("currency") == "USD", "only USD supported")
    require(integer(contract.get("repetitions"), "repetitions") > 0, "repetitions must be positive")
    arms = contract.get("arms", {})
    require(len(arms) >= 2, "at least two arms required")
    common = None
    for identity in arms.values():
        for key in ("model", "model_revision", "effort", "agent_revision", "tool_config_sha256", "memory_snapshot_sha256"):
            require(isinstance(identity.get(key), str) and identity[key].strip(), f"missing arm identity: {key}")
        for key in ("tool_config_sha256", "memory_snapshot_sha256"):
            require(re.fullmatch(r"[0-9a-f]{64}", identity[key]) is not None, f"invalid {key}")
        parity = tuple(identity[key] for key in ("model", "model_revision", "effort", "agent_revision"))
        require(common is None or parity == common, "model/effort/agent identity parity mismatch")
        common = parity
    cases = contract.get("cases", [])
    require(cases and len({case["id"] for case in cases}) == len(cases), "cases must be nonempty and unique")
    for case in cases:
        require(case.get("state") in ("cold", "warm"), "case state must be cold or warm")
        require(isinstance(case.get("workload"), str) and case["workload"], "workload required")
        for key in ("prompt", "source", "history", "evaluator"):
            artifact(root, case[key])
    for price in contract.get("prices", {}).values():
        require(price.get("model") and price.get("source"), "price model and source required")
        date.fromisoformat(price["effective_date"])
        rates = price.get("usd_per_million_tokens", {})
        require(not set(rates) - set(UNITS), "unknown rate unit")
        for rate in rates.values():
            amount(rate)
    require(contract.get("execution_policy") and contract.get("scoring_policy"), "execution/scoring policies required")
...
def evaluate(bundle):
    ...
    require(lock["accountant_sha256"] == digest(__file__), "accountant changed after freeze")
    validate_contract(root, contract)
    cases = {case["id"]: case for case in contract["cases"]}
    arms = contract["arms"]
    expected = {(arm, case, trial) for arm in arms for case in cases
                for trial in range(contract["repetitions"])}
    events = [parse_json(line) for line in (root / "events.jsonl").read_text().splitlines() if line.strip()]
    ...
    for event in events:
        ...
        require(isinstance(event_id, str) and event_id and event_id not in seen, "missing/duplicate event id")
        seen.add(event_id)
        arm = event.get("arm")
        ...
        if kind == "outcome":
            require(key in expected and key not in outcomes, "duplicate or unplanned outcome")
            require(event.get("status") in ("success", "failure", "timeout", "aborted"), "invalid outcome")
        ...
        if kind == "overhead_coverage":
            ...
            require(event.get("status") in ("complete", "incomplete"), "invalid overhead coverage")
            artifact(root, event["evidence"])
        ...
        if kind == "request":
            require(key in expected or (event.get("case") is None and phase != "agent"), "agent request needs task/trial")
            ...
            if response.get("model"):
                require(response["model"] == price["model"], "response/price model mismatch")
            if response.get("usage") is not None:
                require(response.get("model") == price["model"], "usage requires exact response model")
                response_id = response.get("id")
                require(isinstance(response_id, str) and response_id, "usage requires provider response id")
                require((arm, response_id) not in response_ids, "duplicate provider response id")
                response_ids.add((arm, response_id))
            if phase == "agent":
                require(price["model"] == arms[arm]["model"], "agent model differs from contract")
            ...
            rows.append({"id": event_id, "arm": arm, "case": event.get("case"), "trial": event.get("trial"),
                         "phase": phase, "usage": normalized, "usd": str(cost) if cost is not None else None,
                         "elapsed_ms": elapsed,
                         "request_sha256": digest(request_path), "response_sha256": digest(response_path)})
        elif kind == "overhead":
            artifact(root, event["evidence"])
            require(event.get("basis") in ("measured", "estimated"), "overhead basis required")
            cost = amount(event["usd"]) if event.get("usd") is not None else None
            rows.append({"id": event_id, "arm": arm, "phase": phase, "usd": str(cost) if cost is not None else None,
                         "basis": event["basis"]})
        else:
    ...
    for arm in arms:
        ...
        complete = not missing and not no_requests and not unknown[arm] and overhead.get(arm) == "complete"
        reports[arm] = {"planned_tasks": len(arm_expected), "recorded_outcomes": len(arm_expected) - len(missing),
                        "successes": successes, "success_rate": successes / len(arm_expected),
                        "outcome_counts": {status: sum(outcomes.get(key) == status for key in arm_expected)
                                           for status in ("success", "failure", "timeout", "aborted")},
                        "accounting_complete": complete, "known_usd": str(totals[arm]),
                        "total_usd": str(totals[arm]) if complete else None,
                        "usd_per_success": str(totals[arm] / successes) if complete and successes else None,
                        "unknown_cost_events": unknown[arm], "missing_outcomes": missing,
                        "tasks_without_requests": no_requests, "overhead_coverage": overhead.get(arm, "missing"),
                        "known_usd_by_phase": {phase: str(value) for phase, value in phases[arm].items()}}
        reports[arm]["request_count"] = sum(count for key, count in requests.items() if key[0] == arm)
        ...
        reports[arm]["known_token_units"] = {
            unit: sum(row["usage"]["tokens"][unit] for row in rows
                      if row["arm"] == arm and row.get("usage") is not None) for unit in UNITS}
    return {"version": VERSION, "evidence_kind": contract["evidence_kind"],
            "contract_sha256": lock["contract_sha256"], "events_sha256": digest(root / "events.jsonl"),
            "arms": reports, "cost_events": rows,
            "limitations": ["Offline export accounting; completeness declarations are not an independent provider audit.",
