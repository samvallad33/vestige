...
pub fn project(storage: &Storage, opts: &ProjectionOptions) -> Result<Projection> {
    ...
    while region.len() > 10_000 && !items.is_empty() {
        items.pop();
        region = render(opts.format, &opts.scope, &items);
    }
    Ok(Projection {
        format: opts.format,
...
mod tests {
    ...
    #[test]
    fn writes_reject_changed_snapshot_and_preserve_current_text() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("MEMORY.md");
        std::fs::write(&path, "human edit").unwrap();
        assert!(write_projection(&path, Some("old text"), "replacement").is_err());
        assert_eq!(std::fs::read_to_string(&path).unwrap(), "human edit");
        write_projection(&path, Some("human edit"), "updated").unwrap();
