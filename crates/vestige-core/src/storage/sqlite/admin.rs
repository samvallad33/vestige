//! Admin storage operations, extracted from the integrated v3 implementation.
...
impl SqliteMemoryStore {
    ...
    pub(super) fn read_effective_pragmas(conn: &Connection) -> Result<SqliteConnectionPragmas> {
    ...
    pub(super) fn count_foreign_key_violations(conn: &Connection) -> Result<u64> {
    ...
    pub(super) fn repair_cascade_orphans(conn: &Connection) -> Result<u64> {
    ...
    pub(super) fn quick_check_rows(conn: &Connection) -> Result<Vec<String>> {
        let mut out = Vec::new();
    ...
    pub(super) fn fts5_table_names(conn: &Connection) -> Result<Vec<String>> {
    ...
    pub(super) fn quick_check_failure_is_only_fts5(rows: &[String], fts_tables: &[String]) -> bool {
    ...
    pub(super) fn checkpoint_connection(
        conn: &Connection,
    ...
    pub(super) fn scan_last_backup_timestamp(backup_dir: &Path) -> Option<DateTime<Utc>> {
