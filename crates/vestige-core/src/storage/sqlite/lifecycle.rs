//! Lifecycle storage operations, extracted from the integrated v3 implementation.
...
impl SqliteMemoryStore {
    ...
    pub(super) fn get_fsrs_w20(&self) -> Result<f64> {
        let reader = self
            .reader
            .lock()
            .map_err(|_| StorageError::Init("Reader lock poisoned".into()))?;
    ...
    pub(super) fn repair_legacy_passive_retrieval_state(&self) -> Result<i64> {
        let writer = self
            .writer
            .lock()
            .map_err(|_| StorageError::Init("Writer lock poisoned".into()))?;
    ...
    pub(super) fn prune_access_log(&self) -> Result<i64> {
    ...
    pub(super) fn optimize_w20_if_ready(&self) -> Result<Option<f64>> {
