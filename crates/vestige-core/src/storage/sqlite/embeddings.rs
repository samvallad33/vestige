//! Embeddings storage operations, extracted from the integrated v3 implementation.
...
impl SqliteMemoryStore {
    ...
    #[cfg(feature = "vector-search")]
    pub(super) fn vector_search_disable_requested() -> bool {
        #[cfg(test)]
    ...
    pub(super) fn profile_state_text(state: EmbeddingProfileState) -> Result<String> {
    ...
    pub(super) fn migration_state_text(state: EmbeddingMigrationState) -> Result<String> {
        serde_json::to_value(state)
            .map_err(|error| StorageError::InvalidEmbeddingProfile(error.to_string()))?
            .as_str()
            .map(str::to_string)
    ...
    pub(super) fn parse_rfc3339(value: String, field: &str) -> Result<DateTime<Utc>> {
    ...
    #[cfg(all(feature = "embeddings", feature = "vector-search"))]
    pub(super) fn attached_embedder_for(
        &self,
        profile_id: &EmbeddingProfileId,
    ...
    #[cfg(all(feature = "embeddings", feature = "vector-search"))]
    pub(super) fn vector_refresh_plan(
        snapshot: &Connection,
        profile_id: &str,
        last_seq: i64,
    ...
    #[cfg(feature = "vector-search")]
    pub(super) fn reset_vector_index_watermark(&self, journal_seq: i64) {
        if let Ok(mut watermark) = self.vector_index_watermark.lock() {
