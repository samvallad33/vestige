...
class PacketCache:
    ...
    def accept(self, request, response):
        ...
        update = {
            "packetId": packet_id if complete else None,
            "notModified": False,
            "evidenceIncomplete": not complete,
            "results": copy.deepcopy(cards),
        }
        # Dissent metadata belongs with the evidence, not in an optional log.
        if "contradictionProtected" in response:
            update["contradictionProtected"] = copy.deepcopy(
                response["contradictionProtected"]
            )
        if complete:
